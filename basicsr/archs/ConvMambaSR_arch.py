...
class MSSA(BaseModule):
    def __init__(self,
                 channels,
                 kernel_sizes=[3, [1, 7], [1, 13], [1, 17]],
                 paddings=[1, [0, 3], [0, 6], [0, 8]]):
...
class GlobalDetailReconstructionModule(nn.Module):
    def __init__(self, in_ch):
        super(GlobalDetailReconstructionModule, self).__init__()
        self.mamba_chan_attn = simam_module()
        self.conv_spat_attn = MSSA(in_ch)
        self.conv1 = nn.Conv2d(in_ch, in_ch, 3, 1, 1)
        self.conv2 = nn.Conv2d(in_ch, in_ch, 3, 1, 1)

    def forward(self, conv_x, mamba_x):
        conv_spat = self.conv1(self.conv_spat_attn(conv_x))
        mamba_chan = self.conv2(self.mamba_chan_attn(mamba_x))

        x = conv_spat + mamba_chan
        return x
...
class ConvMambaSR(nn.Module):
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(ConvMambaSR, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        for i_layer in range(self.num_layers): # 4-layer
            ...
            self.RSSG_layers.append(layer)
        self.norm = norm_layer(self.num_features)

        # conv layers after RSSG
        self.layers_after_RSSG = nn.ModuleList()
        ...
        for i_layer in range(self.num_layers):
            ...
            self.RCG_Layers.append(layer)

        # conv layers after RCG
        self.layers_after_RCG = nn.ModuleList()
        for i_layer in range(self.num_layers):
    ...
    def forward_features(self, x):
        ...
        x_embedding = self.patch_embed(x) # N,L,C
        x_embedding = self.pos_drop(x_embedding)

        for i in range(self.num_layers):
            x_embedding = self.RSSG_layers[i](x_embedding, x_size)
            Mamba_conv = self.layers_after_RSSG[i](self.patch_unembed(self.norm(x_embedding), x_size))

            conv = self.RCG_Layers[i](x)
            conv_conv = self.layers_after_RCG[i](conv)

            x = self.GDRM_Layers[i](conv_conv, Mamba_conv)
